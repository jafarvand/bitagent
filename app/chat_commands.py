...
class ToolDefinition(BaseModel):
    ...
    approval_policy: Literal[
        "none", "explicit_confirmation", "single_approval", "maker_checker", "prohibited"
    ] = "none"
...
TOOL_REGISTRY: dict[str, ToolDefinition] = {
    ...
    "management.calculate_report": ToolDefinition(
        name="management.calculate_report",
        intent="calculate",
    ...
    "management.generate_report": ToolDefinition(
        name="management.generate_report",
    ...
    "workflow.approve": ToolDefinition(
        ...
        required_roles=["approver", "manager", "admin"],
        risk="medium",
    ...
    "process.pause": ToolDefinition(
        ...
        required_fields=["process_id", "reason"],
        required_roles=["operator", "manager", "admin"],
        risk="high",
    ...
    "settings.propose": ToolDefinition(
        ...
        required_fields=["setting_name", "new_value", "reason"],
        required_roles=["manager", "admin"],
        risk="high",
        approval_policy="maker_checker",
        reversible=True,
...
def _extract_fields(message: str, intent: Intent) -> dict[str, str]:
    ...
    if intent in {"calculate", "generate_report"}:
        ...
        for period in (
            "today", "yesterday", "last week", "this week", "last month", "30 days"
        ):
...
def plan_command(
    ...
    message: str,
    role: str,
    existing: CommandState | None = None,
) -> dict:
    ...
    if not tool_name:
        ...
        return {
            ...
            "reply": "I could not identify an allowed command. Please state the task and target.",
            "tool_call": None,
        }
    ...
    if role not in tool.required_roles:
        ...
        return {
            "state": state.model_dump(),
            "reply": f"Your role is not authorized for {tool.name}.",
            "tool_call": None,
        }
    ...
    if state.missing_fields:
        ...
        return {
            "state": state.model_dump(),
            "reply": f"I need the following information: {needed}.",
            "tool_call": None,
        }
    ...
    if tool.exchange_write_required:
        return {
            "state": state.model_dump(),
            "reply": (
                "The command is ready for policy and approval processing. "
                "No exchange change has been executed."
            ),
            "tool_call": {
                "tool": tool.name,
